...
main: 
      ...
      ;  bnez r5, printresult ; if t2 != 0, jump to printresult
        ...
        dsub $t3, $t1, $t2      ; subtract integers, $t2 - $t1, put result into $t3

        ;sd $t3, result(r0)
        
        daddi r1, r0, 0         ; add immediate, 0 + r0, put result into r1 
        ld r2, list(r1)         ; load item in list into register r2, which is pointed by r1
        daddi r1, r1, 8         ; add immediate, put the result into r1
        ld r2, list(r1)         ; load item in list into register r2, which is pointed by r1
        daddi r1, r1, 8
        ld r2, list(r1)
        daddi r1, r1, 8
        ld r2, list(r1)         ; repetition of codes above
        halt                    ; stop program
